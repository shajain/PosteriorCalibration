...
class DataGenerator:
    ...
    def allX(self):
        return torch.Tensor(np.concatenate((self.positive_loader.dataset.x, self.unlabeled_loader.dataset.x)))

    def valX(self):
        return torch.Tensor(self.validation_loader.dataset.x)

    def makeVal(self):
        x_positive = self.data_pos(self.n_p / 10).astype(float)
        x_unlabeled,_ = self.data_ul(self.n_u / 10, self.alpha)
        self.validation_loader = DataLoader(Dataset(np.concatenate((x_positive, x_unlabeled))),
            batch_size=self.batch_size, shuffle=True)

    def makePositive(self):
        x_positive = self.data_pos(self.n_p).astype(float)
        y_positive = np.ones_like(x_positive)
        self.positive_loader = DataLoader(Dataset(x_positive, y_positive),
            batch_size=self.batch_size, shuffle=True)

    def makeUnlabeled(self):
        x_unlabeled,_ = self.data_ul(self.n_u, self.alpha)
        ds = Dataset(x_unlabeled.astype(float), None)
        self.unlabeled_loader = DataLoader(ds, batch_size=self.batch_size, shuffle=True)
